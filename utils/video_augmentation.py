...
class ColorJitter(object):
    ...
    def __call__(self, clip):
        t = transforms.RandomApply(
            [transforms.ColorJitter(self.brightness, self.contrast, self.saturation, self.hue)], self.prob
            # not strengthened
        )
        return [t(Image.fromarray(np.uint8(img))) for img in clip]
...
class RandomGrayscale(object):
    ...
    def __call__(self, clip):
        if torch.rand(1) < self.prob:
            return [F.rgb_to_grayscale(img, num_output_channels=F.get_dimensions(img)[0]) for img in clip]
        return [img for img in clip]
...
class RandomGaussianBlur:
    def __init__(self, sigma=[0.1, 2.0], prob=0.1):
        self.sigma = sigma
        self.prob = prob
    def __call__(self, clip):
        t = GaussianBlur(self.sigma)
        if torch.rand(1) < self.prob:
            return [t(img) for img in clip]
        return clip

class PIL2ndarray:
    def __call__(self, clip):
        return [np.array(img) for img in clip]
