...
import AlphaPose.get_image_keypoint as get_image_keypoint
faulthandler.enable()
import utils
...
class Processor():
    ...
    def load_data(self):
        ...
        if self.arg.dataset == 'CSL':
            ...
            dataset_list = zip(["train", "train_eval", "dev", "test"], [True, False, False, False])
        # if self.arg.saved_dataset:
        #     self.dataset = torch.load(self.arg.saved_dataset)
        getter = get_image_keypoint.Keypoint_getter()
        for idx, (mode, train_flag) in enumerate(dataset_list):
            ...
            # else:
            self.dataset[mode] = self.feeder(gloss_dict=self.gloss_dict, keypoint_getter = getter, kernel_size= self.kernel_sizes, dataset=self.arg.dataset, **arg)
            self.data_loader[mode] = self.build_dataloader(self.dataset[mode], mode, train_flag)
        # torch.save(self.dataset, './dataset/dataset.pth')
