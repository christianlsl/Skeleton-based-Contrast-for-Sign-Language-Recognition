...
from AlphaPose.keypoint_to_heatmap import get_heatmap

sys.path.append("..")
global kernel_sizes 

class BaseFeeder(data.Dataset):
    def __init__(self, prefix, gloss_dict, keypoint_getter, dataset='phoenix2014', drop_ratio=1, num_gloss=-1, mode="train", transform_mode=True,
        ...
        self.keypoint_getter = keypoint_getter
    ...
    def normalize(self, video, label, file_id=None):
        # print(np.ndarray(video[0]).shape)
        video, label = self.data_aug(video, label, file_id)
        keypoints = get_heatmap(video, self.keypoint_getter)         # NORMALIZED(T,224,224)
        # video, label = self.deep_aug(video, label, file_id)
        video = video.float() / 127.5 - 1      # T,3,224,224
        video = video + keypoints.unsqueeze(1)
        torch.clamp(video, min=0, max=1)
        return video, label
    ...
    def deep_transform(self):
        if self.transform_mode == "train":
            print("Apply training transform.")
            return video_augmentation.Compose([
                video_augmentation.ColorJitter(0.4, 0.4, 0.4, 0.1, 0.8),  # not strengthened
                video_augmentation.RandomGrayscale(0.2),
                video_augmentation.RandomGaussianBlur([.1, 2.], 0.5),
                video_augmentation.ToTensor(),
            ])
        else:
            print("Apply testing transform.")
